...
import bossware.mixin.mixins.accessors.IEntityLivingBase;
import bossware.features.modules.client.Managers;
import bossware.features.modules.combat.Killaura;
import bossware.features.modules.player.Blink;
import bossware.features.modules.player.FakePlayer;
import bossware.features.modules.player.Freecam;
...
public class EntityUtil implements Util {
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            if (BossWare.friendManager.isFriend(player)) {
                continue;
            }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || (entity instanceof EntityPlayer && BossWare.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && BossWare.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = BossWare.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (BossWare.friendManager.isFriend(player) ? "§b" : "§r") + player.getName() + " " + distanceSB.toString() + " " + "§f" + BossWare.totemPopManager.getTotemPopString(player) + BossWare.potionManager.getTextRadarPotion(player), (int) EntityUtil.mc.player.getDistance(player));
