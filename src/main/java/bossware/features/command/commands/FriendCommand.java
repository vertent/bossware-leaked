...
public class FriendCommand
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (BossWare.friendManager.getFriends().isEmpty()) {
                FriendCommand.sendMessage("You currently dont have any friends added.");
            } else {
                FriendCommand.sendMessage("Friends: ");
                for (Map.Entry<String, UUID> entry : BossWare.friendManager.getFriends().entrySet()) {
        ...
        if (commands.length == 2) {
            switch (commands[0]) {
                ...
                default: {
                    FriendCommand.sendMessage(commands[0] + (BossWare.friendManager.isFriend(commands[0]) ? " is friended." : " isnt friended."));
                }
            }
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add": {
                    BossWare.friendManager.addFriend(commands[1]);
                    FriendCommand.sendMessage("\u00a7b" + commands[1] + " has been friended");
                    break;
                }
                case "del": {
                    BossWare.friendManager.removeFriend(commands[1]);
