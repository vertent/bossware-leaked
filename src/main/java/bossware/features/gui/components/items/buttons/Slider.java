...
public class Slider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? BossWare.colorManager.getColorWithAlpha(BossWare.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : BossWare.colorManager.getColorWithAlpha(BossWare.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        BossWare.textManager.drawStringWithShadow(this.getName() + " " + "\u00a77" + (this.setting.getValue() instanceof Float ? (Number) this.setting.getValue() : (Number) ((Number) this.setting.getValue()).doubleValue()), this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), -1);
    }
