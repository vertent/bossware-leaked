...
public class Button
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? BossWare.colorManager.getColorWithAlpha(BossWare.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : BossWare.colorManager.getColorWithAlpha(BossWare.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x20ffffff : -2007673515));
        BossWare.textManager.drawStringWithShadow(this.getName(), this.x + 2.3f, this.y - 2.0f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        BossWare.textManager.drawStringWithShadow(this.getName(), this.x + 2.3f, this.y - 2.0f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
    }
