...
public class Tracer
    ...
    public float[] getColorByDistance(Entity entity) {
        if (entity instanceof EntityPlayer && BossWare.friendManager.isFriend(entity.getName())) {
            return new float[]{0.0f, 0.5f, 1.0f, 1.0f};
        }
        AutoCrystal autoCrystal = BossWare.moduleManager.getModuleByClass(AutoCrystal.class);
