...
import bossware.manager.FileManager;
...
public class AutoGG
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (event.getTarget() instanceof EntityPlayer && !BossWare.friendManager.isFriend(event.getEntityPlayer())) {
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(AutoGG.mc.world) instanceof EntityPlayer && !BossWare.friendManager.isFriend((EntityPlayer) packet.getEntityFromWorld(AutoGG.mc.world))) {
    ...
    public String getRandomMessage() {
        ...
        if (this.messages.size() == 0) {
            return "<player>, Sushi.cc is just tooo strong!";
        }
