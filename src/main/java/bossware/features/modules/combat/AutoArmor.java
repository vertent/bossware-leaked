...
import bossware.features.modules.player.XCarry;
...
public class AutoArmor
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoArmor.mc.currentScreen instanceof PhobosGui) && this.elytraBind.getValue().getBind() == Keyboard.getEventKey()) {
    ...
    public void onTick() {
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue().intValue() * (this.tps.getValue() != false ? BossWare.serverManager.getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {
