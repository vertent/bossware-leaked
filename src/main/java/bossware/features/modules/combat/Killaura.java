...
public class Killaura
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                BossWare.rotationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int wait = this.delay.getValue() == false || EntityUtil.holding32k(Killaura.mc.player) && this.teekaydelay.getValue() == false ? 0 : (wait = (int) ((float) DamageUtil.getCooldownByWeapon(Killaura.mc.player) * (this.tps.getValue() != false ? BossWare.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue().booleanValue()) {
            BossWare.positionManager.setPositionPacket(Killaura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double) target.getEyeHeight(), Killaura.target.posZ, true, true, this.lagBack.getValue() == false);
        }
