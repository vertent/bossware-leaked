...
public class Webaura
    ...
    private boolean check() {
        ...
        return this.target == null || BossWare.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Webaura.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockWeb.class);
    }
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : Webaura.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || trapped && player.isInWeb || EntityUtil.getRoundedBlockPos(Webaura.mc.player).equals(EntityUtil.getRoundedBlockPos(player)) && this.antiSelf.getValue().booleanValue() || BossWare.speedManager.getPlayerSpeed(player) > this.speed.getValue())
                continue;
            if (target == null) {
                target = player;
