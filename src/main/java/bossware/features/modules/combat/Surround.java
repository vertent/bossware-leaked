...
import bossware.features.modules.player.BlockTweaks;
...
public class Surround
    ...
    private final bossware.util.Timer timer = new bossware.util.Timer();
    private final bossware.util.Timer retryTimer = new Timer();
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue() && !BossWare.moduleManager.isModuleEnabled("Freecam")) {
            if (Surround.mc.world.getBlockState(new BlockPos(Surround.mc.player.getPositionVector())).getBlock() == Blocks.WEB) {
                BossWare.positionManager.setPositionPacket(Surround.mc.player.posX, this.startPos.getY(), Surround.mc.player.posZ, true, true, true);
            } else {
                BossWare.positionManager.setPositionPacket((double) this.startPos.getX() + 0.5, this.startPos.getY(), (double) this.startPos.getZ() + 0.5, true, true, true);
            }
        }
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) {
                ...
                case 1: {
                    ...
                    if (!this.extendMove.getValue().booleanValue() && BossWare.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= this.extender.getValue())
                        continue block6;
    ...
    private boolean check() {
        ...
        switch (this.movementMode.getValue()) {
            ...
            case LIMIT: {
                if (!(BossWare.speedManager.getSpeedKpH() > this.speed.getValue())) break;
                return true;
            }
            case OFF: {
                if (!(BossWare.speedManager.getSpeedKpH() > this.speed.getValue())) break;
        ...
        return BossWare.moduleManager.isModuleEnabled("Freecam") || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }
