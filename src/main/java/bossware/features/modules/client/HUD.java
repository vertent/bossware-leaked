...
import bossware.manager.TextManager;
import bossware.util.*;
import bossware.features.modules.Module;
import bossware.features.modules.misc.ToolTips;
import bossware.util.*;
...
public class HUD
    ...
    private final Setting<Boolean> watermark = this.register(new Setting<Boolean>("Logo", false, "WaterMark"));
    private final Setting<Boolean> modeVer = this.register(new Setting<Object>("Version", Boolean.valueOf(false), v -> this.watermark.getValue()));
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "Nsm.club", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public void onUpdate() {
        for (final Module module : BossWare.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        if(this.watermark.getValue()) {
            this.renderer.drawString(BossWare.MODNAME + (this.modeVer.getValue() ? " v" + BossWare.MODVER : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? BossWare.moduleManager.alphabeticallySortedModules.size() : BossWare.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? BossWare.moduleManager.alphabeticallySortedModules.get(k) : BossWare.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00A77" + ((module.getDisplayInfo() != null) ? (" [\u00A7f" + module.getDisplayInfo() + "\u00A77" + "]") : "");
                    final Color moduleColor = BossWare.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? BossWare.moduleManager.alphabeticallySortedModules.size() : BossWare.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? BossWare.moduleManager.alphabeticallySortedModules.get(BossWare.moduleManager.alphabeticallySortedModules.size() - 1 - k) : BossWare.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00A77" + ((module.getDisplayInfo() != null) ? (" [\u00A7f" + module.getDisplayInfo() + "\u00A77" + "]") : "");
                    final Color moduleColor = BossWare.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + "\u00A7f" + BossWare.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : BossWare.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? BossWare.potionManager.getPotionString(effect) : BossWare.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + "\u00A7f" + BossWare.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + "\u00A7f" + BossWare.serverManager.getTPS();
            ...
            final String text = grayString + "Ping " + "\u00A7f" + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : BossWare.serverManager.getPing()) + (this.MS.getValue() ? "ms" : "");
            ...
            final String fpsText = grayString + "FPS " + "\u00A7f" + Minecraft.debugFPS;
            final String text = grayString + "Ping " + "\u00A7f" + BossWare.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue()) {
            BossWare.notificationManager.handleNotifications(height - (k + 16));
        } else {
            BossWare.notificationManager.handleNotifications(height - (j + 16));
        }
        ...
        final String text4 = (this.direction.getValue() ? (BossWare.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to BossWare " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (BossWare.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? "\u00A77" : "\u00A7c") + "Server not responding: " + MathUtil.round(BossWare.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
