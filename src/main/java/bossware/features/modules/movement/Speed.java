...
public class Speed
    ...
    private boolean shouldReturn() {
        return BossWare.moduleManager.isModuleEnabled("Freecam") || BossWare.moduleManager.isModuleEnabled("Phase") || BossWare.moduleManager.isModuleEnabled("ElytraFlight") || BossWare.moduleManager.isModuleEnabled("Strafe") || BossWare.moduleManager.isModuleEnabled("Flight");
    }
        ...
        if (EntityUtil.isEntityMoving(Speed.mc.player) && !Speed.mc.player.collidedHorizontally && !BlockUtil.isBlockAboveEntitySolid(Speed.mc.player) && BlockUtil.isBlockBelowEntitySolid(Speed.mc.player)) {
            ...
            if (Speed.mc.player.posY >= this.startY + this.bounceHeight) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    BossWare.timerManager.setTimer(1.0f);
                }
            }
            if (Speed.mc.player.posY == this.startY) {
                ...
                if (this.useTimer.getValue().booleanValue()) {
                    if (rnd) {
                        BossWare.timerManager.setTimer(1.3f);
                    } else {
                        BossWare.timerManager.setTimer(1.0f);
                    }
                }
