...
import bossware.event.events.*;
import bossware.features.Feature;
import bossware.features.command.Command;
import com.google.common.base.Strings;
import bossware.BossWare;
import bossware.event.events.*;
import bossware.features.modules.client.Managers;
import bossware.features.modules.client.ServerModule;
import bossware.features.modules.combat.AutoCrystal;
import bossware.util.GLUProjection;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            BossWare.potionManager.update();
            BossWare.totemPopManager.onUpdate();
            BossWare.inventoryManager.update();
            BossWare.holeManager.update();
            BossWare.safetyManager.onUpdate();
            BossWare.moduleManager.onUpdate();
            BossWare.timerManager.update();
            if (this.timer.passedMs(Managers.getInstance().moduleListUpdates.getValue().intValue())) {
                BossWare.moduleManager.sortModules(true);
                BossWare.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        BossWare.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        BossWare.moduleManager.onLogout();
        BossWare.totemPopManager.onLogout();
        BossWare.potionManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            BossWare.baritoneManager.onUpdateWalkingPlayer();
            BossWare.speedManager.updateValues();
            BossWare.rotationManager.updateRotations();
            BossWare.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            BossWare.rotationManager.restoreRotations();
            BossWare.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        BossWare.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                BossWare.totemPopManager.onTotemPop(player);
                BossWare.potionManager.onTotemPop(player);
            }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        BossWare.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            BossWare.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            BossWare.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    BossWare.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
