...
import bossware.features.modules.render.XRay;
import bossware.features.setting.Bind;
import bossware.features.setting.EnumConverter;
import bossware.features.setting.Setting;
import bossware.util.Util;
...
public class ConfigManager
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                BossWare.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    BossWare.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void saveConfig(String name) {
        ...
        BossWare.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void init() {
        this.features.addAll(BossWare.moduleManager.modules);
        this.features.add(BossWare.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        BossWare.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            BossWare.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
